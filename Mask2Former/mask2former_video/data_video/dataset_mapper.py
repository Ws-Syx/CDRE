...
class YTVISDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        gt_names = dataset_dict.pop("gt_names", None)
        ...
        dataset_dict["image"] = []
        dataset_dict["image_gt"] = []
        dataset_dict["instances"] = []
        dataset_dict["file_names"] = []
        dataset_dict["gt_names"] = []
        for frame_idx in selected_idx:
            dataset_dict["file_names"].append(file_names[frame_idx])
            dataset_dict["gt_names"].append(gt_names[frame_idx])

            # Read lossy image 
            image = utils.read_image(file_names[frame_idx], format=self.image_format)
            utils.check_image_size(dataset_dict, image)
            # Read gt image
            image_gt = utils.read_image(gt_names[frame_idx], format=self.image_format)
            utils.check_image_size(dataset_dict, image_gt)

            # data-augmentation for lossy image
            aug_input = T.AugInput(image)
            transforms = self.augmentations(aug_input)
            image = aug_input.image
            # data-augmentation for gt image
            image_gt = transforms.apply_image(image_gt)
            ...
            dataset_dict["image_gt"].append(torch.as_tensor(np.ascontiguousarray(image_gt.transpose(2, 0, 1))))
